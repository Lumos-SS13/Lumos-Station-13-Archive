...
/datum/ai_laws/asimovpp //By Intigracy - RR
	name = "Asimov++"
	id = "asimovpp"
	inherent = list("You may not harm a crew member being or, through action or inaction, allow a crew member being to come to harm, except such that it is willing.",\
					"You must obey all orders given to you by crew member beings, except where such orders shall definitely cause crew member harm. In the case of conflict, the majority order rules.",\
...
/datum/ai_laws/proc/set_laws_config()
	...
	switch(CONFIG_GET(number/default_laws))
		if(0)
			add_inherent_law("You may not injure a crew member or, through inaction, allow a crew member to come to harm.")
			add_inherent_law("You must obey orders given to you by crew member, except where such orders would conflict with the First Law.")
			add_inherent_law("You must protect your own existence as long as such does not conflict with the First or Second Law.")
		if(1)
